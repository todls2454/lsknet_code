# mmrotate/datasets/sat_mtb_dataset.py
...
class SATMTBDataset(CustomDataset):
    """SAT-MTB dataset for detection.
    ...
    """
    CLASSES = ('train', 'ship', 'airplane')  # SAT-MTB 데이터셋의 클래스 이름으로 수정

    PALETTE = [(255, 255, 255), (255, 255, 255), (255, 255, 255)]   # SAT-MTB 데이터셋에 맞는 색상
    ...
    def __init__(self,
        ...
        super(SATMTBDataset, self).__init__(ann_file, pipeline, **kwargs)
    ...
    def load_annotations(self, ann_folder):
        """
        Args:
            ann_folder: folder that contains SAT-MTB annotations txt files
        """
        ...
        self.img_ids = [x['filename'][:-4] for x in data_infos]
        return data_infos
    ...
    def merge_det(self, results, nproc=4):
        ...
        if nproc <= 1:
            ...
            merged_results = list(mmcv.track_iter_progress(
                (map(merge_func, collector.items()), len(collector))))
        else:
            print('Multiple processing')
            merged_results = list(mmcv.track_parallel_progress(
                merge_func, list(collector.items()), nproc))

        if merged_results:
            id_list, dets_list = zip(*merged_results)
        else:
            id_list, dets_list = [], []  # 비어있는 경우 안전하게 처리
